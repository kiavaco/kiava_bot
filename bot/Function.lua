...
function a_username_p(arg, data)
	local StartPm = {"↫ ","⇜ ","⌯ ","℘ ","↜ ","⇋ "}
	local M_START = StartPm[math.random(#StartPm)]
	local cmd = arg.cmd
...
function chat_list(msg)
	...
	if #list == 0 then
		message = M_START..'لیست گروهها خالی میباشد'..EndPm
	end
	for k,v in pairs(list) do
		...
		message = message..k..'-'..M_START..'نام گروه : '..GroupsName..'\n'..M_START..'آیدی : ' ..v.. '\n'..M_START..'اعتبار : '..remained_expire..'\n'..M_START..'پنل گروه :\nPanelGp ' ..v.. '\n_______________\n'
	end
...
function action_by_id(arg, data)
	...
	if data.id then
		...
		if cmd == "kick" then
		if tonumber(data.id) == our_id then
			return tdbot.sendMessage(arg.chat_id, "", 0, M_START.."*من نمیتوانم خودم رو از گروه اخراج کنم*"..EndPm, 0, "md")
		elseif is_mod1(arg.chat_id, userid) then
			tdbot.sendMessage(arg.chat_id, "", 0, M_START.."*شما نمیتوانید مدیران،صاحبان گروه و ادمین های ربات رو اخراج کنید*"..EndPm, 0, "md")
		else
			kick_user(data.id, arg.chat_id)
			sleep(1)
			channel_unblock(arg.chat_id, data.id)
		...
		if is_mod1(arg.chat_id, data.id) then
			...
			tdbot.deleteMessagesFromUser(arg.chat_id, data.id, dl_cb, nil)
			tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'تمامی پیام های '..data.id..' پاک شد'..EndPm,17,string.len(data.id))
		end
		end
		if cmd == "banall" then
		if tonumber(data.id) == our_id then
		...
		if is_admin1(data.id) then
			return tdbot.sendMessage(arg.chat_id, "", 0, M_START.."*شما نمیتوانید ادمین های ربات رو از گروه های ربات محروم کنید*"..EndPm, 0, "md")
		end
		if is_gbanned(data.id) then
			tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از گروه های ربات محروم بود'..EndPm,8,string.len(data.id))
		end
		redis:sadd(RedisIndex.."GBanned",data.id)
		kick_user(data.id, arg.chat_id)
		tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از تمام گروه هار ربات محروم شد'..EndPm,8,string.len(data.id))
		end
		if cmd == "unbanall" then
		if not is_gbanned(data.id) then
			tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از گروه های ربات محروم نبود'..EndPm,8,string.len(data.id))
		end
		redis:srem(RedisIndex.."GBanned",data.id)
		tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از محرومیت گروه های ربات خارج شد'..EndPm,8,string.len(data.id))
		end
		if cmd == "ban" then
		if tonumber(data.id) == our_id then
			return tdbot.sendMessage(arg.chat_id, arg.id, 0, M_START.."*من نمیتوانم خودم رو از گروه محروم کنم*"..EndPm, 0, "md")
		end
		if is_mod1(arg.chat_id, tonumber(data.id)) then
			return tdbot.sendMessage(arg.chat_id, "", 0, M_START.."*شما نمیتوانید مدیران،صاحبان گروه و ادمین های ربات رو از گروه محروم کنید*"..EndPm, 0, "md")
		end
		if is_banned(data.id, arg.chat_id) then
			tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از گروه محروم بود'..EndPm,8,string.len(data.id))
		end
		redis:sadd(RedisIndex.."Banned:"..arg.chat_id,data.id)
		kick_user(data.id, arg.chat_id)
		tdbot.sendMention(arg.chat_id,data.id, arg.id,'کاربر '..data.id..' از گروه محروم شد'..EndPm,8,string.len(data.id))
	end
	if cmd == "unban" then
		if not is_banned(data.id, arg.chat_id) then
			tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر '..data.id..' از گروه محروم نبود'..EndPm,8,string.len(data.id))
		end
		redis:srem(RedisIndex.."Banned:"..arg.chat_id,data.id)
		channel_unblock(arg.chat_id, data.id)
		tdbot.sendMention(arg.chat_id,data.id, arg.id,M_START..'کاربر  '..data.id..' از محرومیت گروه خارج شد'..EndPm,8,string.len(data.id))
	end
	if cmd == "silent" then
    if tonumber(data.id) == our_id then
  return tdbot.sendMessage(arg.chat_id, arg.id, 0, M_START.."*من نمیتوانم توانایی چت کردن رو از خودم بگیرم*"..EndPm, 0, "md")
     end
   if is_mod1(arg.chat_id, tonumber(data.id)) then
   return tdbot.sendMessage(arg.chat_id, "", 0, M_START.."*شما نمیتوانید توانایی چت کردن رو از مدیران،صاحبان گروه و ادمین های ربات بگیرید*"..EndPm, 0, "md")
     end
	local function check_silentt(roo, is_silent)
		if is_silent then
				tdbot.sendMention(arg.chat_id,roo.id, arg.id,M_START..'کاربر '..roo.id..' از قبل توانایی چت کردن رو نداشت'..EndPm,8,string.len(roo.id))
		end
			silent_user(arg.chat_id, roo.id)
			tdbot.sendMention(arg.chat_id,roo.id, arg.id,M_START..'کاربر '..roo.id..' توانایی چت کردن رو از دست داد'..EndPm,8,string.len(roo.id))
	end
	...
	local function check_silent(roo, is_silent)
		if not is_silent then
	return tdbot.sendMention(arg.chat_id,roo.id, arg.id,M_START..'کاربر '..roo.id..' از قبل توانایی چت کردن رو داشت'..EndPm,8,string.len(roo.id))
		end
		unsilent_user(arg.chat_id, roo.id)
	 return tdbot.sendMention(arg.chat_id,roo.id, arg.id,M_START..'کاربر '..roo.id..' توانایی چت کردن رو به دست آورد'..EndPm,8,string.len(roo.id))
	end
...
function filter_list(msg)
    ...
    if #names == 0 then
		filterlist = M_START.."`لیست کلمات فیلتر شده خالی است`"..EndPm
    end
...
function modlist(msg)
	...
	if #list == 0 then
	message = M_START.."`در حال حاضر هیچ مدیری برای گروه انتخاب نشده است`"..EndPm
	end
	return message
end
--######################################################################--
function ownerlist(msg)
	...
	if #list == 0 then
		message = M_START.."`در حال حاضر هیچ مالکی برای گروه انتخاب نشده است`"..EndPm
	end
	return message
end
--######################################################################--
--######################################################################--
function AddandRem(msg, mr_roo ,matador)
if not mr_roo and msg.reply_id then
		tdbot_function ({
		_ = "getMessage",
		chat_id = msg.to.id,
		message_id = msg.reply_id
		}, action_by_reply, {chat_id=msg.to.id,cmd=matador})
	end
	if mr_roo and string.match(mr_roo, '^%d+$') then
		tdbot_function ({
		_ = "getUser",
		user_id = mr_roo,
		}, action_by_id, {chat_id=msg.to.id,user_id=mr_roo,cmd=matador})
	end
	if mr_roo and not string.match(mr_roo, '^%d+$') then
		tdbot_function ({
		_ = "searchPublicChat",
		username = mr_roo
		}, action_by_username, {chat_id=msg.to.id,username=mr_roo,cmd=matador})
	end
end