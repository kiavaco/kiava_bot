	...
	if is_channel or is_chat then
		...
		if msg.text then
			if msg.text:match("(.*)") then
				if not redis:get(RedisIndex.."CheckBot:"..msg.to.id) and not redis:get(RedisIndex..auto_leave) and not is_admin(msg) then
		...
		if not is_mod(msg) and not is_whitelist(msg.from.id, msg.to.id) and msg.from.id ~= our_id then
			if redis:get(RedisIndex..'lockgp:'..msg.to.id) then
				Delall(msg)
			end
			if msg.edited then
			...
			if msg.text and redis:get(RedisIndex.."CheckBot:"..msg.to.id) then
				...
				if redis:get(RedisIndex.."CheckBot:"..msg.to.id) then
				max_chars = redis:get(RedisIndex..msg.to.id..'set_char') or 40
			...
			if msg.to.type ~= 'pv' then
				if lock_flood == 'Enable' and not is_mod(msg) and not is_whitelist(msg.from.id, msg.to.id) and not msg.adduser and msg.from.id ~= our_id then
					...
					if redis:get(RedisIndex.."CheckBot:"..msg.to.id) then
						if redis:get(RedisIndex..msg.to.id..'num_msg_max') then
							NUM_MSG_MAX = redis:get(RedisIndex..msg.to.id..'num_msg_max') or 5
						end
					end
